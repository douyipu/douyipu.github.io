...
1. 在消息的最后添加一位比特“1”，这是为了防止当2个不同的消息前面的比特位 相同，而后面有不同个数的0时，如果不在末尾添加比特“1”，那么在填充0后 这两个不同的消息生成的哈希值将会相同。 
...
2. 因为SHA-256哈希函数要将消息进行分块，每块的大小是512比特，所以需要对 输入的数据填充，使其长度是512比特的整数倍。在SHA-256哈希函数的设计中， 最后的64比特用来存储初始消息的比特长度。所以，最后一块中，除了最后添加 的比特“1”和倒数64比特之外，其余位置填充比特“0”。 
